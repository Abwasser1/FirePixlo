...
public class CMD_Start implements Commander, TabCallback {
	...
	public boolean callback(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args.length == 1) {
			...
			if (!list.containsKey(args[0])) {
				V.chat(sender, "§cStart", "§cUnknown Server!");
				return false;
			}
			Server server = list.get(args[0]);
			server.loadWorld();
			V.chat(sender, "§cStart", "§cStarted §3" + server.getName());
		} else {
			chat(sender, "§4Syntax", "§e/start §7<Server>");
		}
		return false;
	}
